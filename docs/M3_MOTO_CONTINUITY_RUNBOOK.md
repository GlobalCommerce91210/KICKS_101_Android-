# M3 — Moto Collector Continuity Validation Runbook

Locked baseline: `89bfe4de8f358ff69f101040528116e68d9ff198` (RC72).

## Safety boundary

- Never uninstall KICK'S from the protected Moto to resolve upgrade problems.
- Never clear app data, collector state, VPN state, or `C:\KICKS` as part of validation.
- Never rewrite collector subject identity.
- Never run destructive migrations against preserved Moto/tablet datasets.
...
Capture before touching the protected Moto installation:

1. Device serial and Android build identity.
...
8. Snapshot/export of relevant `C:\KICKS` dataset metadata without modifying source files.
...
1. Bind the existing device/collector to the DataStorm/KICK'S account.
...
3. Generate a bounded set of normal network activity.
...
1. Reboot the protected Moto normally.
...
M3 is complete only when the protected Moto evidence proves continuity. CI or source inspection alone is not a substitute for the physical-device acceptance criteria.
